...
class Matrix_sub:
    #Tato metoda odecte prvky mezi sebou a vytvori vystupni matici, tak aby ve vystupni matici byly prvky typu float nebo int.
        def sub(self):
            self.output_matrix = []
        ...
        def __init__(self):
            ...
            self.rows = None
            self.cols = None
            self.gui_sub = Toplevel()
            self.gui_sub.geometry("270x80")
            self.rows = Entry(self.gui_sub)
            self.rows.grid(row=3, column=1)
            Label(self.gui_sub, text=' x').grid(row=3, column=2)
            self.cols = Entry(self.gui_sub)
            self.cols.grid(row=3, column=3)
            Button(self.gui_sub, text='Enter',padx=20, pady=4, command=self.matrix_1).grid(row=4, column=3)
